...
def transform_img_tensor(image, config):
    ...
    elif "tiny" in config.classifier:
        image = kornia.geometry.transform.resize(image, 256, interpolation="bicubic")
        image = kornia.geometry.transform.center_crop(image, (64, 64))
        #image = T.Normalize(mean=IMAGENET_DEFAULT_MEAN, std=IMAGENET_DEFAULT_STD)(image)
        image = T.Normalize(mean=[0.5, 0.5, 0.5], std=[0.5, 0.5, 0.5])(image)
    elif "cifar100" in config.classifier:
        image = kornia.geometry.transform.resize(image, 256, interpolation="bicubic")
        image = kornia.geometry.transform.center_crop(image, (32, 32))
        image = T.Normalize(mean=[0.5, 0.5, 0.5], std=[0.5, 0.5, 0.5])(image)

    else:
        image = kornia.geometry.transform.resize(image, 32, interpolation="bicubic")
        #image = kornia.geometry.transform.center_crop(image, (32, 32))
        #image = T.Normalize(mean=IMAGENET_DEFAULT_MEAN, std=IMAGENET_DEFAULT_STD)(image)
        image = T.Normalize(mean=(0.4914,0.4822,0.4465), std=(0.2023,0.1994,0.2010))(image)
    return image
...
def prepare_classifier(config):
    ...
    elif config.classifier =='resnet34_sketch_pacs':
        import torch
        model = torch.hub.load('pytorch/vision:v0.10.0', 'resnet34', pretrained=False)
        model.fc = torch.nn.Linear(512,7)
        #model.load_state_dict(torch.load('../DF_synthesis_LDM/Homework3-PACS/sketch_model_0.174087.pt'))
        model.load_state_dict(torch.load('../DF_synthesis_LDM/Homework3-PACS/lr1e-3_sketch_model_0.237251.pt'))
        model.eval()

    elif config.classifier =='resnet34_from_scratch_cartoon_pacs':
        import torch
        model = torch.hub.load('pytorch/vision:v0.10.0', 'resnet34', pretrained=False)
        model.fc = torch.nn.Linear(512,7)
        model.load_state_dict(torch.load('../DF_synthesis_LDM/Homework3-PACS/from_scratch_lr01_cartoon_model_0.304865.pt'))
        model.eval()

    elif config.classifier =='resnet34_from_scratch_art_pacs':
        import torch
        model = torch.hub.load('pytorch/vision:v0.10.0', 'resnet34', pretrained=False)
        model.fc = torch.nn.Linear(512,7)
        model.load_state_dict(torch.load('../DF_synthesis_LDM/Homework3-PACS/from_scratch_lr01_art_model_0.282296.pt'))
        model.eval()

    elif config.classifier =='resnet34_from_scratch_sketch_pacs':
        import torch
        model = torch.hub.load('pytorch/vision:v0.10.0', 'resnet34', pretrained=False)
        model.fc = torch.nn.Linear(512,7)
        #model.load_state_dict(torch.load('../DF_synthesis_LDM/Homework3-PACS/sketch_model_0.174087.pt'))
